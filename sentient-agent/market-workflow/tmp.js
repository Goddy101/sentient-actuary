...
var VAULT_ADDRESS = "0x72838b813a11691d1B68fBbd32C1eE6961A18022";
var performMacroScan = (nodeRuntime, user, ratioStr, apiKey) => {
  ...
  try {
    ...
    nodeRuntime.log(`⚠️ Live API Error (${e.message}). Using simulated macro-scan.`);
    scanResult = ratio >= 150 ? {
      status: "Volatile",
      riskScore: 82,
      regionTriggered: "NGN/TRY",
      aiReasoning: "Fallback: Elevated inflation prints detected."
    } : {
      status: "Stable",
      riskScore: 24,
      regionTriggered: "None",
      aiReasoning: "Fallback: Emerging market volatility is acceptable."
    };
  }
...
var onLogTrigger = async (runtime2, log) => {
  ...
  try {
    ...
    newRatio = decodedLog.args.newRatio;
  } catch (e) {
    runtime2.log("⚠️ Event Decoding Mismatch: Caught Forwarder event instead of Vault event.");
    runtime2.log("\uD83D\uDEE0 Simulator Mode: Auto-reconstructing 'RatioUpdated' payload for demo...");
    user = "0x984DE9DF12B2c28064234319f5F78acDE0447282";
    newRatio = 160n;
  }
  runtime2.log(`
======================================================`);
  runtime2.log(`\uD83D\uDEA8 EVENT DETECTED: RatioUpdated`);
  runtime2.log(`\uD83D\uDC64 User: ${user}`);
  runtime2.log(`\uD83D\uDCC8 New Collateral Ratio: ${newRatio.toString()}%`);
  runtime2.log(`======================================================
`);
  const rawScanResult = runtime2.runInNodeMode((node) => performMacroScan(node, user, newRatio.toString(), runtime2.config.geminiApiKey), consensusIdenticalAggregation())().result();
  const scanResult = JSON.parse(rawScanResult);
  runtime2.log(`
✅ MACRO SCAN COMPLETE`);
  runtime2.log(`\uD83D\uDCCA Global Status   : ${scanResult.status}`);
  runtime2.log(`\uD83C\uDF21️ Risk Score      : ${scanResult.riskScore}/100`);
  runtime2.log(`\uD83C\uDF0D Active Regions  : ${scanResult.regionTriggered}`);
  runtime2.log(`\uD83D\uDCA1 AI Reasoning    : ${scanResult.aiReasoning}
`);
