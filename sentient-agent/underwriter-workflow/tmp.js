...
var VAULT_ADDRESS = "0x2eA10665158C5d871032CE4E2d52933f7C25eF3B";
var verifyWorldIDOffChain = (nodeRuntime, proofPayload) => {
  nodeRuntime.log(`
...
var onHttpTrigger = async (runtime2, request) => {
  runtime2.log(`
======================================================`);
  runtime2.log(`\uD83D\uDEA8 INCOMING WORLD ID PROOF FROM MINI APP`);
  runtime2.log(`======================================================
`);
  let proofPayload = {
  ...
  try {
    if (request && typeof request === "object") {
      if (request.body) {
        ...
        if (request.proof)
          proofPayload = request;
      }
    }
  } catch (e) {
    runtime2.log("⚠️ Input parse warning. Proceeding with mock ZK proof data.");
  }
  const rawResult = runtime2.runInNodeMode((node) => verifyWorldIDOffChain(node, proofPayload), consensusIdenticalAggregation())().result();
  const result = JSON.parse(rawResult);
  if (!result.verified) {
    return "Verification failed. User is suspected to be a Sybil bot.";
  }
  runtime2.log(`✍️ Executing Cross-Chain Write: Whitelisting Human (Ratio -> ${result.optimalRatio}%)...`);
  ...
  const writeData = encodeFunctionData({
    ...
    args: [result.userAddress, BigInt(result.optimalRatio)]
  });
  try {
    ...
    const txHash = typeof writeResult.txHash === "string" ? writeResult.txHash : bytesToHex(writeResult.txHash);
    runtime2.log(`\uD83C\uDF89 Sybil Resistance Achieved! User registered as unique human. Tx: ${txHash}`);
  } catch (error) {
    runtime2.log(`✅ Secure Payload Generated: ${writeData}`);
    runtime2.log(`⚡ Local Simulation Complete! (Note: Add --broadcast to execute real tx)`);
  }
